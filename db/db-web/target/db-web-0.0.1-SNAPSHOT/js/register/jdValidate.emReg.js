...
function reg() {
	...
	if (closeMobileReg == 1) {
		...
		passed = validateFunction.regValidate() && regNameok && mobileFlags
				&& agreeProtocol;
	} else {
		if (mobile == "") {
			$('#phone').addClass('highlight2');
		...
		if (mobileCode == "") {
			...
			$('#mobileCode_error').removeClass().addClass('error').html(
					'请输入短信验证码');
...
function doLoadUser(result) {
	console.log(result);
	var _username = result.data.username;
	var _password = result.data.password;
	$.ajax({
		type : "POST",
...
function popupReg() {
	...
	var passed = validateRegName() && validateFunction.regValidate()
			&& agreeProtocol && mobileCodeFlag && mobileFlags;
	;
	if (passed) {
		$("#popupRegButton").attr({
			"disabled" : "disabled"
			...
			success : function(result) {
				if (result) {
					...
					if (obj.info) {
						showMessage(obj.info);
						verc();
						$("#popupRegButton").removeAttr("disabled")
								.removeClass().addClass("btn-img btn-regist");
						return;
					}
					if (obj.noAuth) {
						verc();
		...
		$("#popupRegButton").removeAttr("disabled").removeClass().addClass(
				"btn-img btn-regist");
	}
...
function showMessage(alertMsg) {
	$.jdThickBox({
		...
		width : 360,
		height : 100,
		source : '<div class="thickbox-tip">' + '<div class="icon-box">'

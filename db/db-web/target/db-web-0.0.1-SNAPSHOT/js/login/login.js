...
$.extend(validateFunction, {
	...
	FORM_validate : function() {
			...
			$.ajax({
				...
				success : function(result) {
					if (result) {
						var obj = eval(result);
						if (obj.failure) {
							$("#loginpwd_error").html("页面异常，请刷新后重新提交").show()
									.attr({
										"class" : "error"
									});
							return false;
		...
		$("#loginname").jdValidate(validatePrompt.username,
				validateFunction.username, true);
		$("#loginpwd").jdValidate(validatePrompt.pwd, validateFunction.pwd,
				true);
		...
		$.ajax({
			...
			success : function(result) {
				if (result) {
					...
					// if(obj.venture){
					// window.location =
					// "http://safe.jd.com/dangerousVerify/index.action?username="
					// + obj.venture + "&ReturnUrl="+ encodeURI(obj.ventureRet)
					// + "&t=" + new Date().getTime();
					// return;
					// }
					// if (obj.resetpwd) {
					// window.location =
					// "http://safe.jd.com/resetPwd/reset.action?username=" +
					// obj.resetpwd;
					// return;
					// }
					// $("#loginsubmit").removeAttr("disabled");
					...
					// $("#loginname_error").html("您的账号有安全隐患，建议您登录后修改为复杂密码").show().attr({
					// "class": "message" });
					// }
					// if (obj.username) {
					// $("#loginname").attr({ "class": "text highlight2" });
					// $("#loginname_error").html(obj.username).show().attr({
					// "class": "error" });
					// }
					// if (obj.pwd) {
					// $("#nloginpwd").attr({ "class": "text highlight2" });
					// $("#loginpwd_error").html(obj.pwd).show().attr({ "class":
					// "error" });
					// }
					...
					// });
					// $("#authcode_error").html(obj.emptyAuthcode).show().attr({
					// "class": "error" });
					// }
				}
			}
...
function loginNameOk() {
	...
	if (validateRules.isNull(loginName) || loginName == '用户名/邮箱/已验证手机') {
		...
		$("#loginname_error").html("请输入用户名/邮箱/已验证手机").show().attr({
			"class" : "error"
		});
		return false;
	}
						...
						$
								.ajax({
									...
									error : function() {
										...
										$("#loginpwd_error").html("网络超时，请稍后再试")
												.show().attr({
													"class" : "error"
												});
										$("#loginsubmitframe").removeAttr(
												"disabled");
									},
									success : function(result) {
										if (result) {
											...
											if (obj.success || obj.transfer) {
												...
												if (obj.notnr) {
													window.parent.jdModelCallCenter
															.init();
													return;
												}
											...
											if (obj.venture) {
												window.parent.location = "http://safe.jd.com/dangerousVerify/index.action?username="
														+ obj.venture
														+ "&ReturnUrl="
														+ encodeURI(window.parent.location)
														+ "&t="
														+ new Date().getTime();
												return;
											}
											if (obj.resetpwd) {
												window.parent.location = "http://safe.jd.com/resetPwd/reset.action?username="
														+ obj.resetpwd;
												return;
											}
											$("#loginsubmitframe").removeAttr(
											...
											if (obj.verifycode || obj.authcode1
													|| obj.authcode2) {
												$("#o-authcode").show();
											}
											if (obj.authcode2) {
...
function preCheck() {
	...
	if (pin != null && pin.length > 0 && pin != '用户名/邮箱/已验证手机') {
		var url = "../user/preCheck.action?pin=" + escape(pin) + "&r="
				+ Math.random();
		$.getJSON(url, function(result) {
			if (result) {
				...
				if (obj.authcode2) {
					// $("#loginname").attr({ "class":"text highlight2" });
					$("#loginname_error").html("您的账号有安全隐患，建议您登录后修改为复杂密码")
							.show().attr({
								"class" : "message"
							});
				}
...
function showBox() {
	...
	var dwl = "<a href=\"http://static.360buyimg.com/securityctl/"
			+ pgeditor.getLinkHtml() + "\">立即下载</a>";
	jQuery
			.jdThickBox({
				type : "text",
				title : "提示",
				width : 350,
				height : 130,
				source : "<div class=\"step step-0\"><div class=\"mc\"><p>安全控件可提高账户安全性，加密保护您的密码。<a href=\"http://help.jd.com/help/question-61.html\" style=\"color:#005EA7;cursor:pointer\" target=\"_blank\">了解更多&gt;</a></p><ul><li>立即下载</li><li>手动安装</li><li>刷新页面，继续购物</li></ul><div class=\"btn\">"
						+ dwl + "</div></div></div>",
				_autoReposi : true
			});
}
